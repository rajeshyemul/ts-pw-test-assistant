...
- **CHECK STATUS:**
  - **If Status is `BLOCKED`, `FAILED`, or `PASSED WITH WARNINGS`**: STOP immediately. Do NOT create git branches or push code for failed, blocked, or warning-flagged pipeline runs.
  - **If Status is `PASSED` (Clean)**: Proceed to Step 2.
